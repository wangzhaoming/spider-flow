...
public class ProxyPoolCleanTask {
	...
	private static final Logger logger = LoggerFactory.getLogger(ProxyPoolCleanTask.class);

	@Value("${spider.proxypool.clean.enable:true}")
	private boolean enable;
	
	@Scheduled(initialDelay = 10000,fixedDelayString = "${spider.proxypool.clean.delay}")
	public void clean(){
		if(!enable){
			return;
		}
